---
title: 23.6K Star，告别Notion、飞书，开源在线协作云文档平台 Outline 部署指南
cover_image: /img/cover.png
abbrlink: 3c5cdbb2
date: 2024-03-23 14:01:25
---
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/EkhJTM5kB9eTV2uRubSEF5jBJuIDyELhqU5FOgqqRGfTqabvUGtRLIww7EOR8icIpLpDnQnwXgfKuPEes2iaJU8g/640?wx_fmt=png&from=appmsg)一个笔记软件需要使用 X Server 明显超出了合理的范畴，我不想去解决一系列权限和依赖问题，纯属徒增部署难度，果断 Pass
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/EkhJTM5kB9eTV2uRubSEF5jBJuIDyELh4NQ3sUOqvpxJ8BHnbo8VuqWic3Iia5YSYrHMaQ8sicRGlxgCXepxleRVA/640?wx_fmt=png&from=appmsg)现在，三款产品已经排除了两款，已经没有可以选择的了

先介绍下 Outline 有哪些特性：

* 支持团队多人在线协同（跟飞书一样，但是我用不上）
* 支持快速分享链接到互联网上
* 支持 Markdown，还支持引用一些外部块，以及一堆好用的快捷键
* Web UI，适配了移动端与桌面端，随走随用，支持PWA，不用下载客户端（不过也有桌面客户端[4]提供）
![](https://mmbiz.qpic.cn/sz_mmbiz_png/EkhJTM5kB9eTV2uRubSEF5jBJuIDyELhlh4mraRS2a431icRzkncmhAVyQicEtqKFql3VWK5HUYeUnDNcx1cbsvA/640?wx_fmt=png&from=appmsg)* 支持简体中文，拥有完整的本地化支持
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/EkhJTM5kB9eTV2uRubSEF5jBJuIDyELhNib07wibgibAljG90oOfSOoOwVWdAiclxttr4mw5x41VuPLjZicCT8NjVIA/640?wx_fmt=png&from=appmsg)这个问题困扰了我好久，通过不断翻 issue 和 discussion 终于找到一个相关的讨论：https://github.com/outline/outline/pull/2399#issuecomment-916036880
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/EkhJTM5kB9eTV2uRubSEF5jBJuIDyELhzgibibTv0se8J9dmZV1H97XBAkgTh33ZuheS6A3Dadjwf3aZL5BNmyUQ/640?wx_fmt=png&from=appmsg)### 文件迁移
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/EkhJTM5kB9eTV2uRubSEF5jBJuIDyELh8NCS5dZrlNLia8cKoPSmFoWkLF49dtWSNNTbqR38TtrxuRDq3PWbbGg/640?wx_fmt=png&from=appmsg)如果你的 public 和 uploads （可能其中有的文件夹不存在，这取决于你之前上传的文件类型）文件夹没有在根目录，而在二级目录（如 image/uploads ）那么说明上传的时候选中的是 image文件夹，而不是uploads 文件夹，删除重新上传即可，上传完成之后原来的图片理论上就能够正常访问了
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/EkhJTM5kB9eTV2uRubSEF5jBJuIDyELhETPPKH8yvIsGkmI3xxZDShCUibN3eica1c5LoTXXDibryDGm4cybCSN8Q/640?wx_fmt=png&from=appmsg)1. 对象存储域名管理中绑定 CDN 的加速域名（不需要设置 CNAME）（这一步主要是统一回源域名）
![](https://mmbiz.qpic.cn/sz_mmbiz_png/EkhJTM5kB9eTV2uRubSEF5jBJuIDyELh8ZXzolia0BYs4rwtXUic1Iyoc1TBEZwDawnxibwdbUQXYibrZK0MwXwg0A/640?wx_fmt=png&from=appmsg)1. 配置文件中的域名配置为 CDN 的域名
```
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/EkhJTM5kB9eTV2uRubSEF5jBJuIDyELhLk6LL5I1SkskiasvmmickibKgRrVNMuXMOIUeFrOaRHNtIrcCRG0fnzrA/640?wx_fmt=png&from=appmsg)参考资料[1]Appflowy: https://github.com/AppFlowy-IO/AppFlowy
